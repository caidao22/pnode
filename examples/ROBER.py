...
parser.add_argument('--pnode_method', type=str, choices=['euler', 'rk2', 'fixed_bosh3', 'rk4', 'fixed_dopri5', 'beuler', 'cn'], default='cn')
...
def visualize(t, true_y, pred_y, odefunc, itr, name):
    if args.viz:
        ...
        ax1.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, 0], color='tab:orange', linestyle='solid', linewidth=lw, label='Ground Truth', **marker_style1)
        ax1.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, 0], color='tab:blue', linestyle='dashed', linewidth=lw, label=name, **marker_style2)
        ax1.legend()
        ax1.set_xscale('log')
        ...
        ax2.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, 1], color='tab:orange', linestyle='solid', linewidth=lw, **marker_style1)
        ax2.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, 1], color='tab:blue', linestyle='dashed', linewidth=lw, **marker_style2)
        ax2.set_xscale('log')

        ax3.cla()
        ax3.set_xlabel('t')
        ax3.set_ylabel(r'$u_3$')
        ax3.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, 2], color='tab:orange', linestyle='solid', linewidth=lw, **marker_style1)
        ax3.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, 2], color='tab:blue', linestyle='dashed', linewidth=lw, **marker_style2)
        ax3.set_xscale('log')
...
if __name__ == '__main__':
    ...
    ode_PNODE.setupTS(true_y0, func_PNODE, step_size=step_size, method=args.pnode_method, enable_adjoint=True, implicit_form=args.implicit_form)
    optimizer_PNODE = optim.AdamW(func_PNODE.parameters(), lr=5e-3)
    ode_test_PNODE = petsc_adjoint.ODEPetsc()
    ode_test_PNODE.setupTS(true_y0, func_PNODE, step_size=step_size, method=args.pnode_method, enable_adjoint=False, implicit_form=args.implicit_form)
