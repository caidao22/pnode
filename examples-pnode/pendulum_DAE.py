...
t = torch.linspace(0.0, 0.5, args.data_size + 1, dtype=torch.float64)
...
if not args.petsc_ts_adapt:
    ...
    t_traj = torch.linspace(
        ...
        steps=args.data_size + 1 + (args.data_size) * (args.steps_per_data_point - 1),
    )
...
if __name__ == "__main__":
    ...
    if args.pretrained:
        optimizer_PNODE = optim.AdamW(
            filter(lambda p: p.requires_grad, func_PNODE.parameters()), lr=args.lr
        )
    else:
        optimizer_PNODE = optim.AdamW(func_PNODE.parameters(), lr=args.lr)
    if args.lr_scheduler:
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer_PNODE, milestones=[1000, 10000]
        )
    else:
        scheduler = None
    ...
    for itr in range(curr_iter, args.niters + 1):
        ...
        if args.unknown_alg:
            loss_PNODE = loss_PNODE + lam * torch.sum(
                torch.square(func_PNODE.net(pred_y_PNODE)[:, -1])
            )
        loss_PNODE.backward()
        ...
        deviation_save[itr - 1] = torch.sum(
            torch.square(func_PNODE.net(pred_y_PNODE)[:, -1])
        )
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                print(
                    "LR: {:g} Weight_Decay: {:g}".format(
                        optimizer_PNODE.param_groups[0]["lr"],
                        optimizer_PNODE.param_groups[0]["weight_decay"],
                    )
                )
                if args.unknown_alg:
                    print(
                        "Check algebraic constraints, total squared deviation: {:g}".format(
                            deviation_save[itr - 1].item()
                        )
                    )
                ...
                if new_best:
                    ...
                    print(
                        "Saved new best results (loss={:.6f}) at Iter {}".format(
                            best_loss, itr
                        )
                    )
                ii += 1
