...
import os
import pickle
from distances_kNN import (
    apply_dist_KNN,
    apply_dist_threshold,
    get_distances,
    apply_feat_KNN,
)
...
def get_two_hop(data):
    print(
        "raw data contains {} edges and {} nodes".format(data.num_edges, data.num_nodes)
    )
    th = TwoHop()
    data = th(data)
    print(
        "following rewiring data contains {} edges and {} nodes".format(
...
def apply_gdc(data, opt, type="combined"):
    ...
    print(
        "performing gdc transformation with method {}, sparsification {}".format(
            opt["gdc_method"], opt["gdc_sparsification"]
        )
    )
    ...
    if opt["self_loop_weight"] != 0:
        gdc = GDCWrapper(
            float(opt["self_loop_weight"]),
            normalization_in="sym",
            normalization_out="col",
            diffusion_kwargs=diff_args,
            sparsification_kwargs=sparse_args,
            exact=opt["exact"],
        )
    else:
        gdc = GDCWrapper(
...
def make_symmetric(data):
    ...
    ei, ew = get_rw_adj(ei, edge_weight=ew, norm_dim=1, fill_value=0.0, num_nodes=n)
...
def edge_sampling(model, z, opt):
    if opt["edge_sampling_space"] == "attention":
        ...
        threshold = torch.quantile(mean_att, opt["edge_sampling_rmv"])
        mask = mean_att >= threshold
    elif opt["edge_sampling_space"] in [
        "pos_distance",
        "z_distance",
        "pos_distance_QK",
        "z_distance_QK",
        ...
        model.opt["attention_type"] = model.opt[
            "edge_sampling_space"
        ]  # this changes the opt at all levels as opt is assigment link
        pos_enc_distances = model.odeblock.get_attention_weights(
            z
        )  # forward pass of multihead_att_layer
    ...
    if opt["edge_sampling_sym"]:
        model.odeblock.odefunc.edge_index = to_undirected(
            model.odeblock.odefunc.edge_index
        )
...
def add_outgoing_attention_edges(model, M):
    ...
    importance = scatter(atts, dst, dim=0, dim_size=model.num_nodes, reduce="sum").to(
        model.device
    )  # column sum to represent outgoing importance
    degree = scatter(
        torch.ones(size=atts.shape, device=model.device),
        dst,
        dim=0,
        dim_size=model.num_nodes,
        reduce="sum",
    )
    ...
    anchors2 = torch.multinomial(
        torch.ones(model.num_nodes, device=model.device), M, replacement=True
    ).to(model.device)

    new_edges = torch.cat(
        [
            torch.stack([anchors, anchors2], dim=0),
            torch.stack([anchors2, anchors], dim=0),
        ],
        dim=1,
    )
    return new_edges
...
def add_edges(model, opt):
    ...
    if opt["edge_sampling_add_type"] == "random":
        ...
        cat = torch.cat(
            [model.odeblock.odefunc.edge_index, new_edges, new_edges2], dim=1
        )
    ...
    new_ei = torch.unique(
        cat, sorted=False, return_inverse=False, return_counts=False, dim=1
    )
    return new_ei
...
def apply_beltrami(data, opt, data_dir=f"{ROOT_DIR}/data"):
    ...
    # - otherwise, calculate...
        ...
        if opt["pos_enc_type"] == "GDC":
            ...
            print(
                f"[x] The positional encoding type you specified ({opt['pos_enc_type']}) does not exist"
            )
            quit()
        ...
        if opt["pos_enc_csv"]:
            ...
            table_mat = np.concatenate(
                [sp.indices(), np.atleast_2d(sp.values())], axis=0
            ).T
...
def apply_pos_dist_rewire(data, opt, data_dir="../data"):
    if opt["pos_enc_type"].startswith("HYP"):
        ...
        # do encodings already exist on disk?
        fname = os.path.join(
            pos_enc_dir, f"{opt['dataset']}_{opt['pos_enc_type']}_dists.pkl"
        )
...
class GDCWrapper(GDC):
    def __init__(
        ...
        super(GDCWrapper, self).__init__(
            self_loop_weight,
            normalization_in,
            normalization_out,
            diffusion_kwargs,
