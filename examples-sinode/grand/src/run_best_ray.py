...
def get_best_params_dir(opt):
    ...
    best_params_dir = df.sort_values("accuracy", ascending=False)["logdir"].iloc[
        opt["index"]
    ]
...
def run_best_params(opt):
    ...
    result = tune.run(
        ...
        num_samples=(
            opt["reps"] if opt["num_splits"] == 0 else opt["num_splits"] * opt["reps"]
        ),
        scheduler=None,
    ...
    df = result.dataframe(metric=opt["metric"], mode="max").sort_values(
        opt["metric"], ascending=False
    )
    try:
        df.to_csv(
            "../ray_results/{}_{}.csv".format(name, time.strftime("%Y%m%d-%H%M%S"))
        )
    except:
        pass
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--folder", type=str, default=None, help="experiment folder to read"
    )
    parser.add_argument(
        "--index", type=int, default=0, help="index to take from experiment folder"
    )
    parser.add_argument(
        "--metric",
        type=str,
