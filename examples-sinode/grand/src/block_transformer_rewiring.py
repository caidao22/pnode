...
class RewireAttODEblock(ODEblock):
    def __init__(
        ...
        super(RewireAttODEblock, self).__init__(
            odefunc, regularization_fns, opt, data, device, t
        )
        assert (
            opt["att_samp_pct"] > 0 and opt["att_samp_pct"] <= 1
        ), "attention sampling threshold must be in (0,1]"
        self.opt = opt
        ...
        self.odefunc.edge_index = edge_index.to(
            device
        )  # this will be changed by attention scores
        self.odefunc.edge_weight = edge_weight.to(device)
        self.reg_odefunc.odefunc.edge_index, self.reg_odefunc.odefunc.edge_weight = (
    ...
    def get_attention_weights(self, x):
        if self.opt["function"] not in {"GAT", "transformer"}:
            ...
            attention, values = self.odefunc.multihead_att_layer(
                x, self.data_edge_index
            )
        return attention
    ...
    def renormalise_attention(self, attention):
        ...
        att_sums = scatter(
            attention, index, dim=0, dim_size=self.num_nodes, reduce="sum"
        )[index]
    ...
    def add_random_edges(self):
        ...
        with torch.no_grad():
            new_edges = np.random.choice(
                self.num_nodes, size=(2, M), replace=True, p=None
            )
    ...
    def add_khop_edges(self, k=2, rm_self_loops=True):
        n = self.num_nodes
        for i in range(k - 1):
            new_edges, new_weights = torch_sparse.spspmm(
                self.odefunc.edge_index,
                self.odefunc.edge_weight,
                self.odefunc.edge_index,
                self.odefunc.edge_weight,
    ...
    def threshold_edges(self, x, threshold):
        ...
        print(
            "retaining {} of {} edges".format(
                self.odefunc.edge_index.shape[1], self.data_edge_index.shape[1]
            )
        )
    ...
    def forward(self, x):
        ...
        if self.training:
            with torch.no_grad():
                ...
                attention_weights = self.get_attention_weights(x)
                self.odefunc.attention_weights = attention_weights.mean(
                    dim=1, keepdim=False
                )
                ...
                threshold = torch.quantile(
                    self.odefunc.edge_weight, 1 / (pc_change - self.opt["rw_addD"])
                )
