...
def train_ray_rand(opt, checkpoint_dir=None, data_dir="../data"):
    ...
    for split in range(opt["num_splits"]):
        ...
        if opt["baseline"]:
            ...
            model, data = CGNN(opt, adj, opt["time"], device).to(
                device
    ...
    for epoch in range(1, opt["epoch"]):
        loss = np.mean(
            [
                train_this(model, optimizer, data)
                for model, optimizer, data in zip(models, optimizers, datas)
            ]
        )
...
def train_ray_int(opt, checkpoint_dir=None, data_dir="../data"):
    ...
    model = (
        GNN(opt, dataset, device) if opt["no_early"] else GNNEarly(opt, dataset, device)
    )
    if torch.cuda.device_count() > 1:
        model = nn.DataParallel(model)
    model, data = model.to(device), dataset.data.to(device)
...
def set_cora_search_space(opt):
    ...
    opt["hidden_dim"] = tune.sample_from(
        lambda _: 2 ** np.random.randint(6, 8)
    )  # hidden dim of X in dX/dt
    ...
    if opt["block"] in {"attention", "mixed"} or opt["function"] in {
        "GAT",
        "transformer",
        "dorsey",
    }:
        opt["heads"] = tune.sample_from(lambda _: 2 ** np.random.randint(0, 4))  #
        opt["attention_dim"] = tune.sample_from(
            lambda _: 2 ** np.random.randint(4, 8)
        )  # hidden dim for attention
        ...
        opt["self_loop_weight"] = tune.choice(
            [0, 1]
        )  # whether or not to use self-loops
    else:
        opt["self_loop_weight"] = tune.uniform(0, 3)

    opt["tol_scale"] = tune.loguniform(
        1, 1000
    )  # num you multiply the default rtol and atol by
    if opt["adjoint"]:
...
def set_pubmed_search_space(opt):
    ...
    if opt["block"] in {"attention", "mixed"} or opt["function"] in {
        ...
        opt["self_loop_weight"] = (
            tune.choice([0, 0.5, 1, 2])
            if opt["block"] == "mixed"
            else tune.choice([0, 1])
        )  # whether or not to use self-loops
    else:
        opt["self_loop_weight"] = tune.uniform(0, 3)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--time", type=float, default=1.0, help="End time of ODE function."
    )
    parser.add_argument(
        "--augment",
        action="store_true",
    ...
    parser.add_argument(
        "--block", type=str, default="constant", help="constant, mixed, attention, SDE"
    )
    parser.add_argument(
        "--function",
        type=str,
    ...
    # ray args
    parser.add_argument(
        "--num_samples", type=int, default=20, help="number of ray trials"
    )
    parser.add_argument(
        "--gpus",
        type=float,
    ...
    parser.add_argument(
        "--baseline",
        action="store_true",
        help="Wheather to run the ICML baseline or not.",
    )
    parser.add_argument(
        "--regularise",
        dest="regularise",
