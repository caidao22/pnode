...
class myODEFuncTransformerAtt(ODEFunc):
    ...
    def __init__(self, in_features, out_features, opt, data, device):
        ...
        if opt["self_loop_weight"] > 0:
            self.edge_index, self.edge_weight = add_remaining_self_loops(
...
class SpGraphTransAttentionLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.opt["beltrami"] and self.opt["attention_type"] == "exp_kernel":
            ...
            self.Qx = nn.Linear(
                opt["hidden_dim"] - opt["pos_enc_hidden_dim"], self.attention_dim
            )
    ...
    def forward(self, x, edge):
        ...
        if self.opt["beltrami"] and self.opt["attention_type"] == "exp_kernel":
            ...
            x = torch.cat(
                (x[:, : self.opt["feat_hidden_dim"]], x[:, label_index:]), dim=1
            )
            ...
            prods = (
                self.output_var_x**2
                * torch.exp(
                    -torch.sum((src_x - dst_x) ** 2, dim=1)
                    / (2 * self.lengthscale_x**2)
                )
                * self.output_var_p**2
                * torch.exp(
                    -torch.sum((src_p - dst_p) ** 2, dim=1)
                    / (2 * self.lengthscale_p**2)
                )
            )
