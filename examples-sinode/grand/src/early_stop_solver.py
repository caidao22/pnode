...
class EarlyStopDopri5(RKAdaptiveStepsizeODESolver):
    ...
    def evaluate(self, rkstate):
        ...
        if self.dataset == "ogbn-arxiv":
            ...
            loss = self.lf(
                z[self.data.train_mask], self.data.y.squeeze()[self.data.train_mask]
            )
        else:
...
class EarlyStopRK4(FixedGridODESolver):
    ...
    def _step_func(self, func, t, dt, t1, y):
        ver = (
            torchdiffeq.__version__[0]
            + torchdiffeq.__version__[2]
            + torchdiffeq.__version__[4]
        )
        if int(ver) >= 22:  # '0.2.2'
...
SOLVERS = {"dopri5": EarlyStopDopri5, "rk4": EarlyStopRK4}
...
class EarlyStopInt(torch.nn.Module):
    def __init__(self, t, opt, device=None):
        ...
        self.m2_bias = None
        self.opt = opt
        self.t = torch.tensor([0, opt["earlystopxT"] * t], dtype=torch.float).to(
            self.device
        )
    ...
    def __call__(
        ...
        assert method in [
            "rk4",
            "dopri5",
        ], "Only dopri5 and rk4 implemented with early stopping"

        ver = torchdiffeq.__version__
        if (
            int(ver[0] + ver[2] + ver[4]) >= 20
        ):  # 0.2.0 change of signature on this release for event_fn
            event_fn = None
            (
                shapes,
                func,
                ...
                func, y0, self.t, rtol, atol, method, options, event_fn, SOLVERS
            )
        else:
            shapes, func, y0, t, rtol, atol, method, options = _check_inputs(
                func, y0, self.t, rtol, atol, method, options, SOLVERS
            )

        self.solver = SOLVERS[method](
            func, y0, rtol=rtol, atol=atol, opt=self.opt, **options
        )
