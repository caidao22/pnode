...
class ConstantODEblock(ODEblock):
    def __init__(
        self, odefunc, regularization_fns, opt, data, device, t=torch.tensor([0, 1])
    ):
    ...
    def add_random_edges(self):
        ...
        with torch.no_grad():
            ...
            no_repeats = torch.unique(
                cat, sorted=False, return_inverse=False, return_counts=False, dim=0
            )
    ...
    def forward(self, x):
        ...
        if self.training:
            if self.opt["new_edges"] == "random":
                ...
                self.opt["new_edges"] == "random_walk"
                and self.odefunc.attention_weights is not None
            ):
                self.add_rw_edges()
