...
class GNNKNNEarly(BaseGNN):
    def __init__(self, opt, dataset, device=torch.device("cpu")):
        ...
        self.odeblock = block(
            self.f, self.regularization_fns, opt, dataset.data, device, t=time_tensor
        ).to(device)
    ...
    def set_solver_m2(self):
        self.odeblock.test_integrator.m2_weight = (
            self.m2.weight.data.detach().clone().to(self.device)
        )
        self.odeblock.test_integrator.m2_bias = (
            self.m2.bias.data.detach().clone().to(self.device)
        )
    ...
    def forward(self, x, pos_encoding):
        ...
        if self.opt["beltrami"]:
            ...
            x = self.mx(x)
            p = F.dropout(
                pos_encoding, self.opt["input_dropout"], training=self.training
            )
            p = self.mp(p)
...
def main(opt):
    ...
    print(
        "best val accuracy {:03f} with test accuracy {:03f} at epoch {:d}".format(
            best_val_acc, test_acc, best_epoch
        )
    )
    print(
        "best in integrator val accuracy {:03f} with test accuracy {:03f} at epoch {:d}".format(
            best_val_acc_int, test_acc_int, best_epoch_int
        )
    )
