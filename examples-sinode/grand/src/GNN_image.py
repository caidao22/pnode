...
class GNN_image(BaseGNN):
    def __init__(self, opt, data, num_classes, device=torch.device("cpu")):
        ...
        self.odeblocks = nn.ModuleList(
            [
                ...
                for dummy_i in range(self.n_ode_blocks)
            ]
        ).to(self.device)
        self.odeblock = self.block(
            self.f, self.regularization_fns, opt, self.data, device, t=time_tensor
        ).to(self.device)

        self.m2 = nn.Linear(
            opt["im_width"] * opt["im_height"] * opt["im_chan"], num_classes
        )
    ...
    def forward(self, x):
        ...
        z = z.view(
            -1, self.opt["im_width"] * self.opt["im_height"] * self.opt["im_chan"]
        )
    ...
    def forward_plot_path(self, x, frames):  # stitch together ODE integrations
        ...
        for f in range(frames):
            ...
            print(
                f"{torch.sum(z[1:self.opt['im_width'] * self.opt['im_height'] * self.opt['im_chan'], :])}"
            )
            print(
                f"{torch.sum(z[2:self.opt['im_width'] * self.opt['im_height'] * self.opt['im_chan'], :])}"
            )
