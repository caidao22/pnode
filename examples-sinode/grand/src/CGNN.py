...
class CGNN(nn.Module):
    def __init__(self, opt, adj, time, device):
        ...
        self.odeblock = ODEblock(
            ODEFunc(2 * opt["hidden_dim"], 2 * opt["hidden_dim"], opt, adj),
...
def main(opt):
    ...
    model, data = CGNN(opt, adj, opt["time"], device).to(device), dataset.data.to(
        device
    )
    print(opt)
...
def run_best_params(opt):
    ...
    reporter = CLIReporter(metric_columns=["accuracy", "loss", "training_iteration"])
    if opt["dataset"] == "Cora":
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--iter", type=int, default=10, help="Number of training iterations."
    )
    parser.add_argument(
        "--use_gold",
        type=int,
    ...
    parser.add_argument(
        "--tau", type=float, default=1.0, help="Annealing temperature in sampling."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="the number of random weight initialisations to use",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of random slpits >= 0. 0 for planetoid split",
    )
