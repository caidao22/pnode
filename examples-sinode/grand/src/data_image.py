...
def get_image_opt(opt):
    ...
    assert (opt["train_size"]) % opt[
        "batch_size"
    ] == 0, "train_size needs to be multiple of batch_size"
    assert (opt["test_size"]) % opt[
        "batch_size"
    ] == 0, "test_size needs to be multiple of batch_size"
    ...
    opt["num_nodes"] = (
        opt["im_height"] * opt["im_width"] * opt["im_chan"]
    )  # 2708  ###pixels
...
def edge_index_calc(im_height, im_width, im_chan, diags=False):
        ...
        for i in range(im_height * im_width):
            ...
            if i in [
                ...
                if i == 0:
                    ...
                    (
                        edge_list.append([i, im_height * im_width - 2 * im_width + 1])
                        if diags == True
                        else 0
                    )
                    ...
                    (
                        edge_list.append([i, im_height * im_width - im_width - 2])
                        if diags == True
                        else 0
                    )
            # top edge
    ...
    if diags:
        assert ret_edge_tensor.shape[1] == (
            8 * (im_width - 2) * (im_height - 2)
            + 2 * 5 * (im_width - 2)
            + 2 * 5 * (im_height - 2)
            + 4 * 3
        ), "Wrong number of fixed grid edges (inc diags)"
    else:
        assert ret_edge_tensor.shape[1] == (
            4 * (im_width - 2) * (im_height - 2)
            + 2 * 3 * (im_width - 2)
            + 2 * 3 * (im_height - 2)
            + 4 * 2
        ), "Wrong number of fixed grid edges (exc diags)"
...
def create_in_memory_dataset(
    ...
    class IMAGE_IN_MEM(InMemoryDataset):
        def __init__(self, root, transform=None, pre_transform=None, pre_filter=None):
            super(IMAGE_IN_MEM, self).__init__(
        ...
        def process(self):
            ...
            for batch_idx, (data, target) in enumerate(data_loader):
                if type == "GNN":
                    ...
                    y = (
                        self.tensor
                    )  # <- hack the datset num_classes property (code above)
                elif type == "Train":
                    if (
                        opt["testing_code"] == True
                        and batch_idx > opt["train_size"] - 1
                    ):
                        break
                    y = target
                elif type == "Test":
...
def load_data(opt):
    ...
    if opt["im_dataset"] == "MNIST":
        ...
        data_train = torchvision.datasets.MNIST(
            "../data/" + exdataset + "/", train=True, download=True, transform=transform
        )
    ...
    if opt["testing_code"] == True:
        Graph_GNN = create_in_memory_dataset(
            opt,
            "GNN",
            train_loader,
            edge_index,
        ...
        Graph_train = create_in_memory_dataset(
            opt,
            "Train",
            train_loader,
            edge_index,
            ...
            processed_file_name="Graph"
            + exdataset
            + str(opt["train_size"])
            + "Train.pt",
        )
        Graph_test = create_in_memory_dataset(
            opt,
            "Test",
            test_loader,
            edge_index,
            ...
            processed_file_name="Graph" + exdataset + str(opt["test_size"]) + "Test.pt",
        )
    else:
        Graph_GNN = create_in_memory_dataset(
            opt,
        ...
        def process(self):
            ...
            if type == "GNN":
                ...
                adj_mat_train = np.zeros(
                    (
                        coords_train.shape[0],
                        coords_train.shape[1],
                        coords_train.shape[2],
                    )
                )
                ...
                path_test_vals = (
                    os.path.join(
                        path_main,
                        "datasets/mnist_superpixels_data_%d/test_vals.mat" % n_supPix,
                    ),
                )
                path_coords_test = (
                    os.path.join(
                        path_main,
                        "datasets/mnist_superpixels_data_%d/test_patch_coords.mat"
                        % n_supPix,
                    ),
                )
                path_test_labels = (
...
def load_Superpix75Mat(
    opt,
):  # , path='../data/SuperMNIST/MNIST/datasets/mnist_superpixels_data_75/'):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hidden_dim", type=int, default=16, help="Hidden dimension."
    )  ######## NEED
    parser.add_argument(
    ...
    parser.add_argument(
        "--epoch", type=int, default=10, help="Number of training epochs per iteration."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        default="ode",
        help="set ode block. Either 'ode', 'att', 'sde'",
    )  ######## NEED
    parser.add_argument(
        "--adjoint",
        default=False,
    ...
    # SDE args
    parser.add_argument(
        "--dt_min", type=float, default=1e-5, help="minimum timestep for the SDE solver"
    )
    parser.add_argument("--dt", type=float, default=1e-3, help="fixed step size")
    parser.add_argument(
        "--adaptive", type=bool, default=False, help="use adaptive step sizes"
    )
    # Attention args
    parser.add_argument(
        "--leaky_relu_slope",
