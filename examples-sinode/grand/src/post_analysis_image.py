...
def print_image_T(model, dataset, opt, modelpath, height=2, width=3):
    ...
    for i in range(height * width):
        ...
        if opt["im_dataset"] == "MNIST":
            plt.imshow(
                batch.x[torch.nonzero(mask)].view(
                    model.opt["im_height"], model.opt["im_width"]
                ),
                cmap="gray",
                interpolation="none",
            )
        elif opt["im_dataset"] == "CIFAR":
            A = batch.x[torch.nonzero(mask)].view(
                model.opt["im_height"], model.opt["im_width"], model.opt["im_chan"]
            )
            A = A / 2 + 0.5
            plt.imshow(A)
        ...
        if opt["im_dataset"] == "MNIST":
            plt.imshow(
                out[i, :].view(model.opt["im_height"], model.opt["im_width"]),
                cmap="gray",
                interpolation="none",
            )
        elif opt["im_dataset"] == "CIFAR":
...
def print_image_path(model, dataset, opt, height, width, frames):
    ...
    for i in range(height * width):
        ...
        if opt["im_dataset"] == "MNIST":
            plt.imshow(
                paths[i, 0, :].view(model.opt["im_height"], model.opt["im_width"]),
                cmap="gray",
                interpolation="none",
            )
        elif opt["im_dataset"] == "CIFAR":
    ...
    def update(ii):
        for i in range(height * width):
            ...
            if opt["im_dataset"] == "MNIST":
                plt.imshow(
                    paths[i, ii, :].view(model.opt["im_height"], model.opt["im_width"]),
                    cmap="gray",
                    interpolation="none",
                )
            elif opt["im_dataset"] == "CIFAR":
    ...
    animation = FuncAnimation(
        fig, func=update, frames=frames, interval=10
    )  # , blit=True)
    return animation
...
def plot_att_heat(model, model_key, modelpath):
    ...
    dense_att = to_dense_adj(
        edge_index=edge_index, edge_attr=edge_weight, max_num_nodes=num_nodes
    )
