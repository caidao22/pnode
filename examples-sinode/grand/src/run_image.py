...
from post_analysis_image import (
    print_image_T,
    print_image_path,
    plot_att_heat,
    plot_att_edges,
...
def get_image_opt(opt):
    ...
    opt["num_nodes"] = (
        opt["im_height"] * opt["im_width"]
    )  # * opt['im_chan'] #2708  ###pixels
...
def train(epoch, model, optimizer, dataset):
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if (
            batch_idx > model.opt["train_size"] // model.opt["batch_size"]
        ):  # only do this for 1st batch/epoch
            break
        ...
        if batch_idx % 1 == 0:
            print(
                "Batch Index {}, number of function evals {} in time {}".format(
                    batch_idx, model.fm.sum, time.time() - start_time
                )
            )
...
def test(model, dataset):
    ...
    for batch_idx, batch in enumerate(loader):
        if (
            batch_idx > model.opt["test_size"] // model.opt["batch_size"]
        ):  # only do this for 1st batch/epoch
            break
        model.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--block", type=str, default="constant", help="constant, mixed, attention"
    )
    parser.add_argument(
        "--function",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=5000,
        help="Maximum number of function evaluations allowed.",
    )
    parser.add_argument(
        "--reweight_attention",
        type=bool,
