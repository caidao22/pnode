...
def main(opt):
    ...
    model = Node2Vec(
        ...
        walk_length=opt["walk_length"],
        context_size=opt["context_size"],
        walks_per_node=opt["walks_per_node"],
        num_negative_samples=opt["neg_pos_ratio"],
        p=1,
        q=1,
        sparse=True,
    ).to(device)
    ...
    for epoch in range(1, opt["epochs"] + 1):
        ...
        test_t = time.time() - t
        print(
            f"Epoch: {epoch:02d}, Train: {train_t:.2f}, Test: {test_t:.2f},  Loss: {loss:.4f}, Acc: {acc:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--walk_length",
        type=int,
        default=20,  # note this can grow much bigger (paper: 40~100)
        help="Walk length",
    )
    parser.add_argument(
        "--context_size",
        type=int,
        default=16,  # paper shows increased perf until 16
        help="Context size",
    )
    parser.add_argument(
        "--walks_per_node",
        type=int,
        default=16,  # best paper results with 18
        help="Walks per node",
    )
    parser.add_argument(
        "--neg_pos_ratio",
